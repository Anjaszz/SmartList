  ...
  return (
    <>
      ...
      <div className="actions bg-[#3572EF] w-full py-6 text-center">
        ...
        <button onClick={onClear} className="bg-[#A1DD70] text-[#fefae0] mt-3 font-bold rounded-[3rem] py-3 px-4 sm:px-6 md:px-8 text-base sm:text-lg md:text-xl cursor-pointer">
          Bersihkan Daftar
        </button>
      </div>
    </>
...
function Item({ item, onDelete, onChecked }) {
  return (
    <li key={item.id} className="text-[#232323] text-xl sm:text-2xl md:text-3xl font-bold flex items-center mb-6 sm:mb-10 md:mb-14">
      ...
      <button onClick={() => onDelete(item.id)} className="flex items-center justify-center cursor-pointer text-lg sm:text-xl md:text-2xl p-0 ml-[1rem]">
        ❌
      </button>
    </li>
  );
